...
public class angryBirds {
	...
	/*
	 * TO DO: Reduce Buffering/Flashing by finding a way to paste all shapes at once
	 * 
	 * 
	 * 
	 */
...
//		m.mouseClicked(null);
	...
	// Simple program that shrinks and grows a square. Used for testing graphics
	// features.
	public static void graphicsTesting(JFrame f) {
		...
		Graphics2D g2d = (Graphics2D) g;
		shapeList renderQueue = new shapeList(maxSize, maxSize);
	...
	// Self made Graphics loading list so layering can happen :)
	/*
	 * static class shapeList { Graphics g; Graphics2D g2d; Shape[] shapeList;
	 * Color[] colorList; boolean[] fillOrDraw; int shapeCount = 0; int[]
	 * renderOrder; int width, height; Color bgColor = new Color(255,255,255);
	 * boolean updateOnChanges = true;
	 * 
	 * 
	 * 
	 * shapeList(Graphics gWindow, int w, int h) { if (gWindow == null) { return; }
	 * if (w < 0 || h < 0) { return; } width = w; height = h; g = gWindow; g2d =
	 * (Graphics2D)g; //A dummy shape to start off the program so the arrays aren't
	 * all null shapeList = append(shapeList, new Rectangle2D.Double(0,0,1,1));
	 * colorList = append(colorList, new Color(255,255,255)); fillOrDraw =
	 * append(fillOrDraw, true); renderOrder = append(renderOrder, 0); shapeCount++;
	 * } int draw(Shape s, Color c) { if (s == null || c == null) { return -1; }
	 * shapeList = append(shapeList, s); colorList = append(colorList, c);
	 * fillOrDraw = append(fillOrDraw, false); renderOrder = append(renderOrder,
	 * shapeCount); shapeCount++; return (shapeCount - 1); } int fill(Shape s, Color
	 * c) { if (s == null || c == null) { return -1; } shapeList = append(shapeList,
	 * s); colorList = append(colorList, c); fillOrDraw = append(fillOrDraw, true);
	 * renderOrder = append(renderOrder, shapeCount); shapeCount++; return
	 * (shapeCount - 1); } //True for fill, false for draw void replace(int index,
	 * Shape s, boolean isFilled) {
	 * 
	 * 
	 * shapeList[index] = s; fillOrDraw[index] = isFilled; }
	 * 
	 * void setBG(Color c) { bgColor = c; }
	 * 
	 * Shape getShapeByID(int ID) { return shapeList[ID]; } boolean
	 * getFillOrDrawByID(int ID) { return fillOrDraw[ID]; } void renderShapes() {
	 * //The screen business is to avoid buffering. //(The name bufferedImage
	 * doesn't have anything to do with buffering, it's just an image for our use
	 * cases) BufferedImage screen = new BufferedImage(width, height,
	 * BufferedImage.TYPE_INT_RGB); //We get the Graphics object of the image so we
	 * can draw to it Graphics2D tg2d = (Graphics2D)screen.getGraphics();
	 * 
	 * //We draw to the image tg2d.setColor(bgColor); Rectangle2D bg = new
	 * Rectangle2D.Double(0,0,width,height); tg2d.fill(bg); for (int i = 0; i <
	 * shapeCount; i++) { tg2d.setColor(colorList[renderOrder[i]]); if
	 * (fillOrDraw[i]) { tg2d.fill(shapeList[renderOrder[i]]); } else {
	 * tg2d.draw(shapeList[renderOrder[i]]); } }
	 * 
	 * 
	 * //We paste the image onto our Graphics window g.drawImage(screen, 0, 0,
	 * width, height, bgColor, null);
	 * 
	 * } Shape getShape(int id) { return shapeList[id]; } void replaceShape(int id,
	 * int toIndex) { int prevOrder = renderOrder[id]; if (toIndex < prevOrder) {
	 * for (int i = 0; i < renderOrder.length; i++) { if (renderOrder[i] >= toIndex
	 * && renderOrder[i] <= prevOrder) { renderOrder[i]++; } } Shape myShape; //Type
	 * = 1 } else if (prevOrder < toIndex) { for (int i = 0; i < renderOrder.length;
	 * i++) { if (renderOrder[i] <= toIndex && renderOrder[i] >= prevOrder) {
	 * renderOrder[i]--; } } } renderOrder[id] = toIndex; }
	 * 
	 * }
	 */
	...
	/*
	 * static class GraphicObject { enum objType { SHAPEObj, IMAGEObj } objType
	 * thisType;
	 * 
	 * Image thisImage; Shape thisShape;
	 * 
	 * 
	 * GraphicObject(objType myType) { thisType = myType; }
	 * 
	 * void setType(objType myType) {thisType = myType;} void setImage(Image img) {
	 * thisImage = img;} void setShape(Shape shape) {thisShape = shape;}
	 * 
	 * 
	 * Image myImage; //Type = 2 void fill(int type) { if (thisType ==
	 * objType.SHAPEObj) {
	 * 
	 * } } }
	 * 
	 * static Shape[] append(Shape[] arr, Shape value) { if (arr == null) { arr =
	 * new Shape[1]; arr[0] = value; return arr; }
	 * 
	 * Shape[] arry = arr; arr = new Shape[arr.length + 1]; for (int i = 0; i <
	 * arry.length; i++) { arr[i] = arry[i]; } arr[arry.length] = value; return arr;
	 * } static int[] append(int[] arr, int value) { if (arr == null) { arr = new
	 * int[1]; arr[0] = value; return arr; }
	 * 
	 * int[] arry = arr; arr = new int[arr.length + 1]; for (int i = 0; i <
	 * arry.length; i++) { arr[i] = arry[i]; } arr[arry.length] = value; return arr;
	 * }
	 * 
	 * static boolean[] append(boolean[] arr, boolean value) { if (arr == null) {
	 * arr = new boolean[1]; arr[0] = value; return arr; }
	 * 
	 * boolean[] arry = arr; arr = new boolean[arr.length + 1]; for (int i = 0; i <
	 * arry.length; i++) { arr[i] = arry[i]; } arr[arry.length] = value; return arr;
	 * }
	 * 
	 * static Color[] append(Color[] arr, Color value) { if (arr == null) { arr =
	 * new Color[1]; arr[0] = value; return arr; }
	 * 
	 * Color[] arry = arr; arr = new Color[arr.length + 1]; for (int i = 0; i <
	 * arry.length; i++) { arr[i] = arry[i]; } arr[arry.length] = value; return arr;
	 * }
	 */
}
